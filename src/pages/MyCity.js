import React, { useState, useEffect } from "react";
import { fetchWeather } from "../api/weatherApi";

const MyCity = () => {
  const [weather, setWeather] = useState(null);

  useEffect(() => {
   fetchWeather("Znob-Novhorodsk").then(setWeather);
  }, []);

  return (
    <div>
      <h2>Моє місто — Зноб-Новгородське</h2>
<p>
  Зноб-Новгородське — селище в Сумській області. Це невелике, спокійне
  та затишне місце з природою, історією і своїм особливим характером.
</p>
      {weather ? (
        <div>
          <h3>Погода:</h3>
          <p>Температура: {weather.temperature}</p>
          <p>Опис: {weather.description}</p>
          <p>Вітер: {weather.wind}</p>
          <img src={weather.icon} alt="Погода" />
        </div>
      ) : (
        <p>Завантаження погоди...</p>
      )}
    </div>
  );
};

export default MyCity;
